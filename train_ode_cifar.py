...
def get_args():
    ...
    p.add_argument(
        ...
        help="For pulse QAT, apply mismatch to pulse amplitude after quantization or "
...
def _constr_model_name(args, rep=1):
    ...
    if args.run_tag:
        rep_suffix = "_{}REP".format(rep)
        if not model_name.endswith(rep_suffix):
            raise ValueError("Expected model name to end with {}.".format(rep_suffix))
        model_name = model_name[:-len(rep_suffix)] + "_{}{}".format(args.run_tag, rep_suffix)
