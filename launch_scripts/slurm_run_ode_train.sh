      ...
      for distill_temperature in "${DISTILL_TEMPERATURES[@]}"; do
        ...
        jid=$(
          sbatch --parsable \
            ...
            --export=ALL,CONTRAST_METHOD="${contrast_method}",NEG_SAMPLE="${neg_sample}",DISTILL_ALPHA="${distill_alpha}",DISTILL_TEMPERATURE="${distill_temperature}" \
            "${SBATCH_SCRIPT}"
        )
