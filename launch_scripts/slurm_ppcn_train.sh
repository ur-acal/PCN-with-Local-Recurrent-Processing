...
# ( source ./launch_scripts/slurm_ppcn_train.sh ) \
...
### Note: Different from ode train, blocks here is the PCConv to use ###
declare -A BLOCKS_BY_ARCH
# 5L2p - S
BLOCKS_BY_ARCH["5L2PS"]="FFFBReLU6 FFFBReLU6NoLastConv"
# 5L1p - S
BLOCKS_BY_ARCH["5L1PS"]="FFFBReLU6 FFFBReLU6NoLastConv"
# 5L2p - M
BLOCKS_BY_ARCH["5L2PM"]="FFFBReLU6 FFFBReLU6NoLastConv"
# 5L1p - M
BLOCKS_BY_ARCH["5L1PM"]="FFFBReLU6 FFFBReLU6NoLastConv"
# 7L2p - L
BLOCKS_BY_ARCH["7L2PL"]="FFFBReLU6 FFFBReLU6NoLastConv"
# 7L1p - L
BLOCKS_BY_ARCH["7L1PL"]="FFFBReLU6 FFFBReLU6NoLastConv"
# 18L2p - Tiny
BLOCKS_BY_ARCH["18L2PTi"]="FFFBReLU6 FFFBReLU6NoLastConv"
# 18L1p - Tiny
BLOCKS_BY_ARCH["18L1PTi"]="FFFBReLU6 FFFBReLU6NoLastConv"


# Which ARCH/PCN combos to run
FINETUNE="0"
ARCHES=("5L2PS" "5L2PM" "7L2PL")
...
SBATCH_SCRIPT="${REPO_ROOT}/launch_scripts/run_ppcn_train.sbatch"
...
split_and_submit() {
  ...
  while (( i < n )); do
    ...
    ###############################################################################################
    local EXP="no_bn_${pcn}_NODE_1121_scanGFI_ppcn_${arch}_${img_type}_Exp"
    ###############################################################################################
    ...
    jid=$( BLOCKS_LIST="${csv}" \
      sbatch --parsable \
             ...
             --export=ALL,ARCH_SET="${arch}",PCN="${pcn}",IMG_TYPE="${img_type}",FINETUNE="${FINETUNE}",EXP="${EXP}" \
             "${SBATCH_SCRIPT}" )
    echo "  -> job ${jid}"
    JOBS_BY_EXP["${EXP}"]+="${jid}:"
