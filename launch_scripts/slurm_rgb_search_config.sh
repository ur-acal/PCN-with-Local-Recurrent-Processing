...
submit_chunk() {
  ...
  jid=$(
    ...
    sbatch --parsable \
      ...
      --export=ALL,PCN="${pcn}",IMG_TYPE="${img_type}",EXP="${EXP}",FT_MODEL="${FT_MODEL}",CIRC_CONF="${circ_conf}",TASK="${TASK}",ODE_BLOCK="${ODE_BLOCK}",ENABLE_MEASURED_ACTIVATION="${ENABLE_MEASURED_ACTIVATION}",SCALE_MEASURED_ACTIVATION="${SCALE_MEASURED_ACTIVATION}",ACTIVATION_CURVE_PATH="${ACTIVATION_CURVE_PATH}",ACTIVATION_CORNER="${ACTIVATION_CORNER}",ACTIVATION_SPLINE_PARAMETERS="${ACTIVATION_SPLINE_PARAMETERS}",ENABLE_SPIN_VARIATION="${ENABLE_SPIN_VARIATION}",SIGMA_SPIN="${SIGMA_SPIN}",ENABLE_SUMMING_CURRENT_NOISE="${ENABLE_SUMMING_CURRENT_NOISE}",SUMMING_CURRENT_P="${SUMMING_CURRENT_P}",CHAN_0="${chan0}",NUM_LAYERS="${num_layers}",CHUNK_ID="${chunk_id}",CHUNK_TAG="${chunk_tag}",COMB_LIST="${comb_list}" \
      "${SBATCH_SCRIPT}"
  )
  echo "  -> job ${jid}"
  JOBS_BY_EXP["${EXP}"]+="${jid}:"
