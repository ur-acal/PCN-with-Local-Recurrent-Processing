...
CHAN_0_LIST=( 24 26 28 32 34 36 ) # For the default three pattern
...
NUM_LAYERS_BY_CHAN0[24]="6 9"
NUM_LAYERS_BY_CHAN0[26]="6 9"
NUM_LAYERS_BY_CHAN0[28]="6 9"
NUM_LAYERS_BY_CHAN0[30]="6 9"
NUM_LAYERS_BY_CHAN0[32]="6 9"
NUM_LAYERS_BY_CHAN0[34]="6 9"
NUM_LAYERS_BY_CHAN0[36]="6 9"
...
TRAIN_MODE="rgb_search" # "kd_crd_ft", "train_ft", "mix_all", "rgb_search"
...
elif [[ "${TRAIN_MODE}" == "rgb_search" ]]; then
  SBATCH_SCRIPT="${REPO_ROOT}/launch_scripts/run_rgb_search_config.sbatch"
else
...
####################################################
SUMMARY_PKL_OUT="${MERGE_OUT_DIR}/summary_dict_0427_rgb.pkl"
####################################################
...
submit_chunk() {
  ...
  local EXP="0427_${TRAIN_MODE}_${pcn}_NODE_search_${TASK}_${img_type}_${circ_conf:-NoCirc}_C${chan0}_N${num_layers}_${chunk_tag}_chunk${chunk_id}_Exp"
  ...
  jid=$(
    ...
    sbatch --parsable \
      ...
      --export=ALL,PCN="${pcn}",IMG_TYPE="${img_type}",EXP="${EXP}",FT_MODEL="${FT_MODEL}",CIRC_CONF="${circ_conf}",TASK="${TASK}",ODE_BLOCK="${ODE_BLOCK}",ENABLE_MEASURED_ACTIVATION="${ENABLE_MEASURED_ACTIVATION}",ENABLE_PRETRAIN_MEASURED_ACTIVATION="${ENABLE_PRETRAIN_MEASURED_ACTIVATION}",SCALE_MEASURED_ACTIVATION="${SCALE_MEASURED_ACTIVATION}",ACTIVATION_CURVE_PATH="${ACTIVATION_CURVE_PATH}",ACTIVATION_CORNER="${ACTIVATION_CORNER}",ACTIVATION_INTERPOLATION="${ACTIVATION_INTERPOLATION}",ACTIVATION_SPLINE_PARAMETERS="${ACTIVATION_SPLINE_PARAMETERS}",ACTIVATION_FIT_CONSTRAINT="${ACTIVATION_FIT_CONSTRAINT}",ENABLE_SPIN_VARIATION="${ENABLE_SPIN_VARIATION}",SIGMA_SPIN="${SIGMA_SPIN}",ENABLE_SUMMING_CURRENT_NOISE="${ENABLE_SUMMING_CURRENT_NOISE}",SUMMING_CURRENT_P="${SUMMING_CURRENT_P}",CHAN_0="${chan0}",NUM_LAYERS="${num_layers}",CHUNK_ID="${chunk_id}",CHUNK_TAG="${chunk_tag}",COMB_LIST="${comb_list}" \
      "${SBATCH_SCRIPT}"
  )
  echo "  -> job ${jid}"
  JOBS_BY_EXP["${EXP}"]+="${jid}:"
