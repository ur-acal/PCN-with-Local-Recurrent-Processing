...
export EVAL_PARALLELISM="${EVAL_PARALLELISM:-3}"
export SIMULATE="${SIMULATE:-0}"
export CONDITIONS="${CONDITIONS:-max_additive,multiplicative}"
  ...
  for DATASET in "${datasets[@]}"; do
    for SIZE in "${sizes[@]}"; do
      ...
      if [[ "${DRY_RUN:-0}" == 1 ]]; then
        ...
        echo "submitted job ${jid}: row=${ROW}, dataset=${DATASET}, size=${SIZE}, stage=${STAGE}, output=${OUTPUT_ROOT}"
        if [[ "${SIMULATE}" == 1 ]]; then
