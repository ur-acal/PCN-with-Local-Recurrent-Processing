...
# ( source ./launch_scripts/slurm_kd_ode_train.sh ) \
#  > ./logs/scheduler_slurm/kd_scheduler.log 2>&1 < /dev/null &
#
# sched_pid=$!
# disown -h "$sched_pid"
###############################################################################################
...
ARCHES=("DeepTi1P" "Tiny_5L1P" "Tiny_5L2P")
...
SBATCH_SCRIPT="${REPO_ROOT}/launch_scripts/run_kd_ode_train.sbatch"
...
split_and_submit() {
  local arch="$1" pcn="$2" img_type="$3" kd_w="$4" kd_t="$5"
  ...
  while (( i < n )); do
    ...
    ###############################################################################################
    local EXP="no_bn_${pcn}_NODE_1104_KD_${arch}_${img_type}_KDW${kd_w}_KDT${kd_t}_Exp"
    ###############################################################################################

    EXPORTS="ALL"
    EXPORTS+=",ARCH_SET=${arch}"
    EXPORTS+=",PCN=${pcn}"
    EXPORTS+=",IMG_TYPE=${img_type}"
    EXPORTS+=",EXP=${EXP}"
    EXPORTS+=",TEACHER=${TEACHER}"
    EXPORTS+=",T_WRAPPER=${T_WRAPPER}"
    EXPORTS+=",KD_TYPE=${KD_TYPE}"
    EXPORTS+=",KD_W=${kd_w}"
    EXPORTS+=",KD_T=${kd_t}"
  ...
  for ARCH in "${ARCHES[@]}"; do
    for PCN in "${PCNS[@]}"; do
      for KD_W in "${KD_WEIGHTS[@]}"; do
        for KD_T in "${KD_T_LIST[@]}"; do
          split_and_submit "$ARCH" "$PCN" "$IMG_TYPE" "$KD_W" "$KD_T"
        done
      done
    done
  done
