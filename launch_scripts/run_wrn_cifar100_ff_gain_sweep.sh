...
ARCHITECTURES_CSV="${ARCHITECTURES_CSV:-WRN_16_2,WRN_16_4,WRN_28_2,WRN_28_4}"
GAINS_CSV="${GAINS_CSV:-0.90,0.91,0.92,0.93,0.94,0.95,0.96,0.97,0.98,0.99,1.00,1.01,1.02,1.03,1.04,1.05,1.06,1.07,1.08,1.09,1.10}"

IFS=',' read -ra ARCHITECTURES <<<"$ARCHITECTURES_CSV"
IFS=',' read -ra GAINS <<<"$GAINS_CSV"
...
merge_csvs() {
  local filename="$1" destination="$2" expected=$((${#ARCHITECTURES[@]} * ${#GAINS[@]}))
  local files=()
...
{
  "dataset": "cifar100",
  "architectures_csv": "$ARCHITECTURES_CSV",
  "gains_csv": "$GAINS_CSV",
  "checkpoint_root": "$CHECKPOINT_ROOT",
  "mismatch": "none",
  "trials_per_gain": 1,
...
"$PYTHON_BIN" baseline/generate_wrn_ff_gain_summary.py \
