        ...
        if mismatch_levels is not None or noise_level is not None:
            ...
            if levels:
                ...
                self.noisy_model = (
                    ...
                    ) if pulse_training else WrappedNoisyModel(
                        model=self.model,
                        noise_levels=self.noise_schedule,
                        noise_type=effective_noise_type,
                    )
...
class WrappedNoisyPulseModel(WrappedNoisyModel):
    """Mismatch-aware training wrapper for integer-level pulse blocks."""
    ...
    def __init__(self, model, noise_levels, noise_type="mul",
        ...
        if mismatch_mode not in self.MODES:
            raise ValueError(
                "Unsupported pulse mismatch mode={}. Expected one of {}.".format(
                    mismatch_mode, sorted(self.MODES)))
        self.mismatch_mode = mismatch_mode
        self.pulse_blocks = [
            module for module in self.model.modules()
            if getattr(module, "supports_pulse_training", False)]
        if not self.pulse_blocks:
            raise ValueError("WrappedNoisyPulseModel requires at least one pulse-training block.")
