...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Optional zero-based ReLU MC curve index used for every trial.")
    parser.add_argument(
        "--fixed_coupler_mc_index", type=int, default=None,
        help=("Optional zero-based coupler MC curve index used as the only "
              "curve in the selected corner bank."))
    parser.add_argument(
        "--full_45_corner_test",
    ...
    parser.add_argument(
        ...
        default=True)
    parser.add_argument(
        "--full_45_corner_enable_slow_summing_current",
        type=lambda v: v.lower() in ("yes", "true", "t", "1"),
        default=False)
    parser.add_argument(
        "--full_45_corner_enable_slow_coupler_noise",
        type=lambda v: v.lower() in ("yes", "true", "t", "1"),
        default=False)
    parser.add_argument(
    ...
    parser.add_argument("--toggle_timing_mode",
                        choices=("derived", "fixed"), default="derived")
    parser.add_argument("--toggle_y_time", type=float, default=5e-9)
    parser.add_argument("--z_over_y_time", type=float, default=3.0)
    parser.add_argument(
    ...
    parser.add_argument(
        "--full_45_corner_slow_summing_current", type=float, default=2.47e-9)
    parser.add_argument(
        "--full_45_corner_slow_coupler_noise", type=float, default=2.47e-9)
    ...
    parser.add_argument(
        "--input_quant_bits",
        type=lambda s: None if s.lower() in {"none", "", "auto"} else int(s),
        default=None)
    parser.add_argument(
        "--center_student_input",
        type=lambda s: None if s.lower() in {"none", "", "auto"}
        else s.lower() in {"yes", "true", "t", "1"},
        default=None)
...
def build_command(args, case_name):
    ...
    task = "cifar100" if "C100" in args.model_name else "cifar10"
    img_type = "CiFAIR" if "cifair" in args.model_name.lower() else "scanGFI"
    ode_block = (
    ...
    command = [
        ...
        "--enable_slow_summing_current",
        bool_arg(args.enable_slow_summing_current),
        "--slow_summing_current", str(args.slow_summing_current),
        "--enable_slow_coupler_noise", bool_arg(args.enable_slow_coupler_noise),
...
def _corner_trial_configuration(args, corner, corner_index):
    ...
    if args.activation_curve_sharing == "per_model":
        ...
        if args.fixed_relu_mc_index is not None:
            raise ValueError(
                "fixed_relu_mc_index is valid only for per_model ReLU sharing.")
        relu_indices = None
    # Every expanded convolution samples its own shared curve from this
    # corner bank. The trial seed is advanced by ode_inference.
...
def build_corner_command(args, corner, corner_index):
    ...
    if trial_config["coupler_indices"] is not None:
        ...
        curve_bank_indices = corner["coupler"]["curve_indices"]
        if args.fixed_coupler_mc_index is not None:
            if args.fixed_coupler_mc_index not in curve_bank_indices:
                raise ValueError(
                    "Corner {} does not contain coupler MC index {}.".format(
                        corner["id"], args.fixed_coupler_mc_index))
            curve_bank_indices = [args.fixed_coupler_mc_index]
        command.extend([
    ...
    _set_command_arg(
        command, "--enable_slow_summing_current",
        bool_arg(args.full_45_corner_enable_slow_summing_current))
    _set_command_arg(
        command, "--slow_summing_current",
        args.full_45_corner_slow_summing_current)
    _set_command_arg(
        command, "--enable_slow_coupler_noise",
        bool_arg(args.full_45_corner_enable_slow_coupler_noise))
    _set_command_arg(
        command, "--slow_coupler_noise",
        args.full_45_corner_slow_coupler_noise)
    _set_command_arg(
        ...
        for line in process.stdout:
            ...
            if match:
                ...
                result = {
                    ...
                    "activation_curve_sharing": args.activation_curve_sharing,
                    "relu_mc_index": (
                        trial_config["relu_indices"][trial_index]
                        if trial_config["relu_indices"] is not None else None),
                    "coupler_mc_index": (
                        trial_config["coupler_indices"][trial_index]
                        if trial_config["coupler_indices"] is not None
                        else args.fixed_coupler_mc_index),
