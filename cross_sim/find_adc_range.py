...
def find_adc_range(params_args, n_layers, depth):
    ...
    limits_dir = "/home/rongzeng/_workspce_old/repos/pcn/PCN-with-Local-Recurrent-Processing/cross_sim/calibrated_config/"
    ...
    if adc_bits > 0 and adc_range_option == "CALIBRATED":
        ...
        if Nslices == 1:
            ...
            raise ValueError(
                "No calibrated ADC range found for the combination of crossbar style, adc_per_ibit and # slices")

        if adc_ranges[0] is None:
            raise ValueError(
                "For the chosen crossbar settings, calibrated ADC ranges are unavailable for this network: " + \
