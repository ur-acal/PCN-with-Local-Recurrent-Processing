...
class ODEMismatchAnalyzer:
    ...
    # ------------------------------------------------------------------
    # Realistic noisy-prefix analysis
    # ------------------------------------------------------------------

    def _unique_prefix_params(self, layer_idx):
        """
        Parameters before PcConvs[layer_idx].

        This includes optional first_conv stem and previous PcConvs blocks.
        Current layer parameters are intentionally excluded.
        """
        params = []
        ...
        # Optional plain first_conv stem used by PCNetSeparable / PCNetWith1stConv.
        # Weight-only, consistent with _unique_weight_params().
        if hasattr(self.model, "first_conv"):
            add_param(getattr(self.model.first_conv, "weight", None))

        # Previous ODE / PCConv blocks.
        for i in range(layer_idx):
            block = self.model.PcConvs[i]
            for p in self._unique_weight_params(block):
                add_param(p)

        return params

    @torch.no_grad()
    def get_input_to_layer_with_noisy_prefix(self, x, layer_idx, params, deltas):
        """
        Compute input to PcConvs[layer_idx] after applying prefix mismatch.

        Reuses get_input_to_layer(). Parameters are restored before return.
        """
        self._add_param_perturb(params, deltas)

        try:
            x_layer = self.get_input_to_layer(x, layer_idx)
        finally:
            self._remove_param_perturb(params, deltas)
    ...
    @torch.no_grad()
    def analyze_block_realistic(self, layer_idx, images=None):
        """
        Memory-friendly realistic noisy-prefix analysis for one layer.

        This version does NOT concatenate noisy-prefix inputs across seeds.
        It follows the same loop style as _analyze_ode_block().

        For each seed:
            1. sample prefix mismatch
            2. compute x_layer_noisy
            3. sample current-layer mismatch
            4. compute realistic D metrics
            5. compute rho/eta/H/C at x_layer_noisy
        """
        if images is None:
            images, _ = self._get_one_batch()
        ...
        # --------------------------------------------------------------
        # 1. Fully clean reference at this layer.
        # --------------------------------------------------------------
        x_layer_clean = self.get_input_to_layer(images, layer_idx)

        ys_clean_ref, _, T = self._solver_trajectory(block, x_layer_clean)

        yN_clean_ref = self._output_transform(block, ys_clean_ref[-1])
        yN_clean_ref_flat = yN_clean_ref.flatten(1)
        yN_clean_ref_norm = yN_clean_ref_flat.norm(dim=1).clamp_min(self.eps)

        f_clean_ref = block._make_ode_fn(x_layer_clean)
        y0_clean_ref = ys_clean_ref[0]
        t0 = block.integration_time[0].to(
            device=x_layer_clean.device,
            dtype=x_layer_clean.dtype,
        )

        y1_clean_ref_raw = y0_clean_ref + T * f_clean_ref(t0, y0_clean_ref)
        y1_clean_ref = self._output_transform(block, y1_clean_ref_raw)
        y1_clean_ref_flat = y1_clean_ref.flatten(1)
        y1_clean_ref_norm = y1_clean_ref_flat.norm(dim=1).clamp_min(self.eps)

        prefix_params = self._unique_prefix_params(layer_idx)
        current_params = self._unique_weight_params(block)
        ...
        x_layer_clean_norm = x_layer_clean.flatten(1).norm(dim=1).clamp_min(self.eps)

        for _ in range(self.n_seeds):
            # ----------------------------------------------------------
            # 2. Sample prefix mismatch and compute noisy-prefix input.
            # ----------------------------------------------------------
            prefix_deltas = self._sample_deltas(prefix_params)

            x_layer_noisy = self.get_input_to_layer_with_noisy_prefix(
                images,
                layer_idx,
                prefix_params,
                prefix_deltas,
            ...
            prefix_input_distortion_b = (
                (x_layer_noisy.flatten(1) - x_layer_clean.flatten(1)).norm(dim=1)
                / (self.sigma * x_layer_clean_norm)
            )
            prefix_input_distortion_vals.append(prefix_input_distortion_b)

            # ----------------------------------------------------------
            # 3. Clean current-layer reference at noisy-prefix input.
            #
            # This is used for current-layer-only D and rho/eta/C.
            # ----------------------------------------------------------
            ys_noisy_input_clean, h, T = self._solver_trajectory(
                block,
                x_layer_noisy,
            )

            yN_noisy_input_clean = self._output_transform(
                block,
                ys_noisy_input_clean[-1],
            )
            yN_noisy_input_clean_flat = yN_noisy_input_clean.flatten(1)
            yN_noisy_input_clean_norm = (
                yN_noisy_input_clean_flat.norm(dim=1).clamp_min(self.eps)
            )

            f_noisy_input_clean = block._make_ode_fn(x_layer_noisy)
            y0_noisy_input_clean = ys_noisy_input_clean[0]

            y1_noisy_input_clean_raw = (
                y0_noisy_input_clean
                + T * f_noisy_input_clean(t0, y0_noisy_input_clean)
            )
            y1_noisy_input_clean = self._output_transform(
                block,
                y1_noisy_input_clean_raw,
            )
            y1_noisy_input_clean_flat = y1_noisy_input_clean.flatten(1)
            y1_noisy_input_clean_norm = (
                y1_noisy_input_clean_flat.norm(dim=1).clamp_min(self.eps)
            )

            # ----------------------------------------------------------
            # 4. Sample current-layer mismatch.
            # ----------------------------------------------------------
            current_deltas = self._sample_deltas(current_params)

            # ----------------------------------------------------------
            # 5. Total realistic ODE sensitivity.
            #
            # previous layers noisy -> x_layer_noisy
            # current layer noisy -> yN_pert
            # compare against fully clean reference from x_layer_clean
            # ----------------------------------------------------------
            self._add_param_perturb(current_params, current_deltas)

            try:
                ys_pert, _, _ = self._solver_trajectory(block, x_layer_noisy)
                yN_pert = self._output_transform(block, ys_pert[-1])
            finally:
            ...
            try:
                f_pert = block._make_ode_fn(x_layer_noisy)
                y0_pert = block.init_y(x_layer_noisy)
                y1_pert_raw = y0_pert + T * f_pert(t0, y0_pert)
                y1_pert = self._output_transform(block, y1_pert_raw)
            finally:
                self._remove_param_perturb(current_params, current_deltas)
            ...
            # ----------------------------------------------------------
            # 7. Stepwise current-layer contributions v_k at x_layer_noisy.
            #
            # These explain current-layer temporal coherence under realistic
            # noisy-prefix input.
            # ----------------------------------------------------------
            v_list = []

            for k in range(self.n_steps):
                yk_clean = ys_noisy_input_clean[k]
                tk = t0 + k * h
                ...
                vk = (
                    yN_from_k.flatten(1) - yN_noisy_input_clean_flat
                ) / self.sigma
            ...
            V_sum = V.sum(dim=0)
            V_sum_sq = V_sum.pow(2).sum(dim=1)
        ...
        # 8. Aggregate.
        # --------------------------------------------------------------
        H_realistic = H_accum / self.n_seeds

        diag = torch.diag(H_realistic).clamp_min(self.eps).sqrt()
        C_from_H_realistic = H_realistic / (
            diag[:, None] * diag[None, :]
        ).clamp_min(self.eps)

        C_avg_cos_realistic = C_avg_cos_accum / self.n_seeds

        C_from_H_realistic_summary = self._summarize_C(
            C_from_H_realistic,
            "C_from_H_realistic",
        )
        C_avg_cos_realistic_summary = self._summarize_C(
            C_avg_cos_realistic,
            "C_avg_cos_realistic",
        )

        D_ode_realistic_mean = torch.cat(D_ode_realistic_vals).mean()
        D_one_step_realistic_mean = torch.cat(D_one_step_realistic_vals).mean()

        D_ode_current_noisy_prefix_mean = torch.cat(
            D_ode_current_noisy_prefix_vals
        ).mean()
        D_one_step_current_noisy_prefix_mean = torch.cat(
            D_one_step_current_noisy_prefix_vals
        ).mean()

        return {
            # Total realistic distortion:
            # prefix mismatch + current-layer mismatch.
            "D_ode_realistic": D_ode_realistic_mean.item(),
            "D_one_step_realistic": D_one_step_realistic_mean.item(),
            "D_ratio_ode_over_one_step_realistic": (
                D_ode_realistic_mean
            ...
            "D_one_step_current_noisy_prefix": (
                D_one_step_current_noisy_prefix_mean.item()
            ),
            "D_ratio_current_noisy_prefix": (
                D_ode_current_noisy_prefix_mean
            ...
            # Scalar C summaries.
            **C_from_H_realistic_summary,
            **C_avg_cos_realistic_summary,
    ...
    @torch.no_grad()
    def analyze_all_blocks_realistic(self, images=None):
        """
        Same calling style as analyze_all_blocks(images=None),
        but uses realistic noisy-prefix inputs.
        """
        if images is None:
            images, _ = self._get_one_batch()
        ...
        for layer_idx in range(self.model.num_layers):
            results[layer_idx] = self.analyze_block_realistic(
                layer_idx,
                images=images,
            )
