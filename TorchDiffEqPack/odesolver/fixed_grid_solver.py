...
import torch
__all__ = ['Euler','RK2','RK4', 'ProjEuler', 'ProjRK4']
...
class FixedGridSolver(ODESolver):
    ...
    def __init__(self, func, t0, y0, t1=1.0, h=0.1, rtol=1e-3, atol=1e-6, neval_max=500000,
                 ...
                 eps=None, noise_type="mul", noise_generator=None):
...
class ProjRK4(RK4):
    ...
    def step(self, func, t, dt, y, return_variables=False):
        ...
        k2 = func(t + dt / 2.0, tuple( self.proj_fn(_y + 1.0 / 2.0 * dt *_k1) for _y, _k1 in zip(y, k1)  )   )
        k3 = func(t + dt / 2.0, tuple( self.proj_fn(_y + 1.0 / 2.0 * dt *_k2) for _y, _k2 in zip(y, k2)  ) )
        k4 = func(t + dt,  tuple( self.proj_fn(_y + dt *_k3) for _y, _k3 in zip(y, k3)  )   )
        out = tuple( self.proj_fn(_y + 1.0 / 6.0 * dt * _k1 + 1.0 / 3.0 * dt * _k2 + 1.0 / 3.0 * dt * _k3 + 1.0 / 6.0 * dt * _k4)
                     for _y, _k1, _k2, _k3, _k4 in zip(y, k1, k2, k3, k4))
        if return_variables:
            return out, None, [k1, k2, k3, k4]
        else:
